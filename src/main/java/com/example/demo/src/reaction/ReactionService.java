...
public class ReactionService {
    ...
    public void createNewsComment(PostNewsCommentReq postNewsCommentReq) throws BaseException {
        try {
            reactionDao.createNewsComment(postNewsCommentReq);
        } catch (Exception exception) { // DB에 이상이 있는 경우 에러 메시지를 보냅니다.
            throw new BaseException(DATABASE_ERROR);
        }
    }
    ...
    // 뉴스 댓글 삭제
    public void deleteNewsComment(DeleteNewsCommentReq deleteNewsCommentReq) throws BaseException {
        try {
            reactionDao.deleteNewsComment(deleteNewsCommentReq);
